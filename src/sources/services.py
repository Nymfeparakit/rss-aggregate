...
class SourceService(BaseModelService):
    ...
    def __init__(self, db_session, folder_service: UserFolderService):
    ...
    async def create_folder_item(self, item: schemas.FolderItem, user: User):
...
async def get_sources_service(db_session=Depends(get_async_session)):
    folder_service = await get_folders_service(db_session)
